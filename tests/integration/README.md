...
- `tests/integration/cases/*_cases.json` - Test case definitions in JSON format
- `tests/integration/run_mcp_tests.py` - Main test runner script
...
PYTHONPATH=src python tests/integration/run_mcp_tests.py "python src/teradata_mcp_server/server.py"
```

**Using Python with PYTHONPATH (for development from source):**
```bash
PYTHONPATH=src python tests/integration/run_mcp_tests.py "python -m teradata_mcp_server"
```
...
```bash
python tests/integration/run_mcp_tests.py "uv run teradata-mcp-server" "tests/integration/cases/fs_test_cases.json"
```

### Verbose Output
```bash
python tests/integration/run_mcp_tests.py "uv run teradata-mcp-server" --verbose
```
...
python tests/integration/run_mcp_tests.py "python -m teradata_mcp_server --profile fs"
```
## Pass/Fail Logic
