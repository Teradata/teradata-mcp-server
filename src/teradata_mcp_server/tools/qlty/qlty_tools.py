...
def handle_qlty_missingValues(conn: TeradataConnection, database_name: str | None, table_name: str, *args, **kwargs):
    ...
    with conn.cursor() as cur:
        rows = cur.execute(
            f"select ColumnName, NullCount, NullPercentage from TD_ColumnSummary ( on {table_name} as InputTable using TargetColumns ('[:]')) as dt ORDER BY NullCount desc"
        )
...
def handle_qlty_negativeValues(conn: TeradataConnection, database_name: str | None, table_name: str, *args, **kwargs):
    ...
    with conn.cursor() as cur:
        rows = cur.execute(
            f"select ColumnName, NegativeCount from TD_ColumnSummary ( on {table_name} as InputTable using TargetColumns ('[:]')) as dt ORDER BY NegativeCount desc"
        )
        data = rows_to_json(cur.description, rows.fetchall())
        metadata = {
...
def handle_qlty_distinctCategories(
    conn: TeradataConnection, database_name: str | None, table_name: str, column_name: str, *args, **kwargs
):
    """
    ...
    """
    logger.debug(
        f"Tool: handle_qlty_distinctCategories: Args: table_name: {database_name}.{table_name}, column_name: {column_name}"
    )
    ...
    with conn.cursor() as cur:
        rows = cur.execute(
            f"select * from TD_CategoricalSummary ( on {table_name} as InputTable using TargetColumns ('{column_name}')) as dt"
        )
        data = rows_to_json(cur.description, rows.fetchall())
        metadata = {
...
def handle_qlty_standardDeviation(
    ...
    """
    logger.debug(
        f"Tool: handle_qlty_standardDeviation: Args: table_name: {database_name}.{table_name}, column_name: {column_name}"
    )
    ...
    with conn.cursor() as cur:
        rows = cur.execute(
            f"select * from TD_UnivariateStatistics ( on {table_name} as InputTable using TargetColumns ('{column_name}') Stats('MEAN','STD')) as dt ORDER BY 1,2"
        )
        data = rows_to_json(cur.description, rows.fetchall())
        metadata = {
...
def handle_qlty_columnSummary(conn: TeradataConnection, database_name: str | None, table_name: str, *args, **kwargs):
    ...
    with conn.cursor() as cur:
        rows = cur.execute(
            f"select * from TD_ColumnSummary ( on {table_name} as InputTable using TargetColumns ('[:]')) as dt"
        )
        data = rows_to_json(cur.description, rows.fetchall())
        metadata = {
...
def handle_qlty_univariateStatistics(
    ...
    """
    logger.debug(
        f"Tool: handle_qlty_univariateStatistics: Args: table_name: {database_name}.{table_name}, column_name: {column_name}"
    )
    ...
    with conn.cursor() as cur:
        rows = cur.execute(
            f"select * from TD_UnivariateStatistics ( on {table_name} as InputTable using TargetColumns ('{column_name}') Stats('ALL')) as dt ORDER BY 1,2"
        )
        data = rows_to_json(cur.description, rows.fetchall())
        metadata = {
...
def handle_qlty_rowsWithMissingValues(
    ...
    """
    logger.debug(
        f"Tool: handle_qlty_rowsWithMissingValues: Args: table_name: {database_name}.{table_name}, column_name: {column_name}"
    )
    ...
    with conn.cursor() as cur:
        rows = cur.execute(
            f"select * from TD_getRowsWithMissingValues ( ON {table_name} AS InputTable USING TargetColumns ('[{column_name}]')) AS dt;"
        )
        data = rows_to_json(cur.description, rows.fetchall())
        metadata = {
